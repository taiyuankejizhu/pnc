        ...
        for(k1=BX0+3,k2=0,j=0; j<14; j++)
        {
            ...
            if(Table.Index[i+Showline]==1)
            {
                ...
                case    2:
                    ...
                    break;
                case    3:
                    itoa(Table.Dianliu[i+Showline],str,10);
                    break;
                case    4:
...
char *KeyStr(int y,int x)
{
    ...
    char * keystr[0x12][14]=
    {
        ...
        /*5*/   {" 声 音 "," 睡  眠 ","上 升 00"," 关  机 "," 清  时 ","        "," 设  置 "," 返  回 "},
...
void ShowMess(int value)
{
    ...
    if(sl5==1)
    {
        strcpy(showinput1[3],"电流:0~200(A)");
        strcpy(showinput[3],"峰值电流 0~200 (A)");
    }
...
{
    int current[3][3]= {{95,63,31},{191,95,63},{31,31,31}};
    char cur[5];
    int i=0;
    char * showinput1[9]=
    {
...
void ShowTB(char value)
{
    ...
    if(!i)
    {
        bar(CX0+1,CY0+19,CX1-78,CY0+35);
        return;
    }
    if(value>0xf)
    {
...
void ShowF8(int lines)
{
    ...
    if(!lines)
    {
        ...
        Dispcbar(F9X+5-20+150,F9Y-10+21*2,F9X+40+150,F9Y+5+18*2+20,0,color,1,"电 流:");
        Dispcbar(F9X+5-20+150,F9Y-10+21*3,F9X+40+150,F9Y+5+18*3+20,0,color,1,"声 音:");
        Dispcbar(F9X+5-20+150,F9Y-10+21*4,F9X+40+150,F9Y+5+18*4+20,0,color,1,"亮 度:");
    }
}
/*修改运动顺序*/
...
{
    char *str[__LNSTR8__]= {"铜-钢","石墨-钢","铜-钨钢"};
    return str[flag];
}
