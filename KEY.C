...
if(ORgin&0x80){ShowMess(17);return ; }
if(Dis_flag&&K[8])F11();if(UDK1&0x20)DNUP=1;Down();}    /* DN 键  */
else if((F[4]<5||F[4]>6)&&!(UDK1&0x20)){
if(ORgin&0x80){ShowMess(17);return ; }
if(UDK1&0x01)DNUP=2;    /* UP 键  */
if(Dis_flag&&!K[8])F11();Up();}
...
if(UDK&0x20)F11_flag=0;
    ...
    if(Ck8255_flag&0x40){Ck8255_flag&=0xbf;ShowTB(0x0);}
    if(Scr_flag){SoundTime=SoundTIME;return;}
    k=KeyAss();
    while(KeyPress())KeyAss();
    SoundTime=SoundTIME;
    OpenSound();
    Sound_flag=0;
    if(Fx&&k==0x52){K11();outb(OSC,0x01); OTPS0&=0x7f;outportb(OSC+1,OTPS0);}
    #if TESTING
    if(k=='S' || k == 's'){K12();}
    ...
    if(Fx&&k==aF12&&!Dis_flag){if(!F[1]||F[6]>1||KEYL==6){ShowMess(17);return;}else
    {ClearKey(0);F12_flag=0;Dis_flag=1;lock_z=0;
        DN_K=0;UP_K=0;ShowKey(0xff,0xff);
    position_xdis=position_x;position_ydis=position_y;}}

    if(!Fx&&k==0xd)GetInput();
    if(!Fx){
      if(!XYZ[0]&&!XYZ[1]&&!XYZ[2]){
        switch(k){
          case 'x':
          case 'X':
        {MaxL1=MaxL;MaxN1=MaxN;MinN1=MinN;}
        ShowMess(0xfd);X();break;
          case 'y':
          case 'Y':
        {MaxL1=MaxL;MaxN1=MaxN;MinN1=MinN;}
        ShowMess(0xfd);Y();break;
          case 'z':
          case 'Z':
        {MaxL1=MaxL;MaxN1=MaxN;MinN1=MinN;}
        ShowMess(0xfd);Z();break;
        }
      }else{
        i=XYZ[0]|XYZ[1]|XYZ[2];
        switch(k){
          case 'x':
          case 'X':if(XYZ[1])Y();if(XYZ[2])Z();X();break;
          case 'y':
          case 'Y':if(XYZ[0])X();if(XYZ[2])Z();Y();break;
          case 'z':
          case 'Z':if(XYZ[0])X();if(XYZ[1])Y();Z();break;
          case 0xd:
        if(XYZ[0])X();
        if(XYZ[1])Y();
        if(XYZ[2])Z();
        ...
        ClearKey(0);MaxL=MaxL1;MaxN=MaxN1;MinN=MinN1;ShowMess(0xfe);
        }
        return;
      }
    }else{if(KEYL==1&&k==aF4){F6();}
    else  if(XYZ[0]||XYZ[1]||XYZ[2]){
        if(XYZ[0])X();if(XYZ[1])Y();if(XYZ[2])Z();
        ClearKey(0);MaxL=MaxL1;MaxN=MaxN1;MinN=MinN1;ShowMess(0xfe);
        }
    }
  switch(KEYL){
    case 0:
      if(Fx){   /*主菜单*/
    switch(k){
     case aF1:F2();ShowKey(1,1);break;
      case aF2:if(!Pumn_flag){F3();ShowKey(1,2);}else {F[2]=0;ShowKey(1,2);ShowMess(17);}break;
      case aF3:KEYL=1;ShowKey(2,0);break;
      case aF4:KEYL=2;ShowKey(3,0);break;
      case aF5:KEYL=3;ShowKey(4,0);break;
      case aF6:KEYL=4;ShowKey(5,0);break;
      case aF7:K9();ShowKey(1,7);break;
    }return;
      }
      break;
    case 1: /*坐标*/
      if(Fx){
    if(k!=aF2&&F[8]){F9();ShowKey(2,2);}
    if(k!=aF3&&F[9]){F10();ShowKey(2,3);}
    switch(k){
      case aF1:F5();ShowKey(2,1);break;
      case aF2:F9();ShowKey(2,2);break;
      case aF3:F10();ShowKey(2,3);break;
      case aF4:F6();ShowKey(2,4);break;
      case aF5:K4();ShowKey(2,5);ShowKey(2,2);break;
      case aF6:if(!Dis_flag&&!Origin_Flag){KEYL=11;DispF10(0);ShowKey(7,0);}else {if(Dis_flag)ShowMess(17);else {if(Origin_Flag)ShowMess(48);}}break;
      case aF8:KEYL=0;ShowKey(1,0);break;
    }return;
      }if(!Fx&&k==0xd){
    if(F[8])F9();
    if(F[9])F10();
    ...
    case 2: /*放电*/
      if(Fx){
    if(k!=aF2&&F[8]){F9();ShowKey(3,2);}
    if(k!=aF3&&F[9]){F10();ShowKey(3,3);}
    if(k!=aF4&&K[5]){ShowKey(3,4);}
    if(k!=aF5&&F[7]&&F[7]!=3){F8(0);ShowKey(3,5);}
    if(k!=aF6&&P[8]==1){P9();ShowKey(3,6);}
    switch(k){
      case aF1:KEYL=5;KXX=-1;ShowKey(6,0);F7(0);break;
      case aF2:F9();ShowKey(3,2);break;
      case aF3:F10();ShowKey(3,3);break;
      case aF4:return;
      case aF5:F8(0);ShowKey(3,5);break;
      case aF6:P9();ShowKey(3,6);ShowFlag(7);break;
      case aF8:KEYL=0;ShowKey(1,0);break;
    }return;
      }if(!Fx&&k==0xd){
    if(F[8])F9();
    ...
    case 3: /*安全*/
      if(Fx){
    switch(k){
      case aF1:KEYL=9;ShowKey(8,0);break;
      case aF2:P5();ShowKey(4,2);break;
      case aF3:K5();ShowKey(4,3);break;
      case aF4:P4();ShowKey(4,4);break;
      case aF5:P3();ShowKey(4,5);break;
      case aF6:K7();ShowKey(4,6);break;
      case aF8:KEYL=0;ShowKey(1,0);break;
    }return;
      }
      break;
    case 4: /*系统*/
      ...
      if(F[3]&&(k==UP||k==DN))
     {
        if(k==UP)soundV++;
        if(k==DN)soundV--;
        if(soundV<0)soundV=0;
        if(soundV>7)soundV=7;
        WriteSPI(0x1c,soundV);
        Opens_V(soundV);
        ShowKey(5,0);
     }
    if(k!=aF3&&K[2]==1){K3();ShowKey(5,3);}
    if(k!=aF5&&P[1]){P2();ShowKey(5,5);}
    if(k!=aF6&&P[9]){P10();ShowKey(5,6);}
    switch(k){
      case aF1:F4();ShowKey(5,0);break;
      case aF2:K2();ShowKey(5,2);break;
      case aF3:K3();ShowKey(5,3);break;
      case aF4:K10();ShowKey(5,4);break;
      case aF5:P2();ShowKey(5,5);break;
      case aF6:P[14]=0;P10();ShowKey(5,6);ShowKey(5,7);break;
      case aF7:P[9]=0;P16();ShowKey(5,7); break;
      case aF8:KEYL=0;ShowKey(1,0);break;
    }return;
      }
   if(!Fx&&k==0xd)
    ...
    case 5: /*参数*/
      if(Fx){
    switch(k){
/*定段*/  case aF1  :if(!Dis_flag){Dis_lines=0;Dis_start=0;Dis_end=0;ShowTable(0);KEYL=7;ShowKey(7,0);}else ShowMess(17);break;
/*自动*/  case aF2  :ShowInput(0);if(!Dis_flag){KEYL=6;Y7Y=1;ShowKey(7,0);DispF7(0);P[13]=1;}else ShowMess(17);return;
/*删除*/  case aF3  :if(!Dis_flag){Table.Index[YY]=0;DelLine();}else ShowMess(17);break;
/*修底*/  case aF4  :P11();ShowKey(6,4);break;
      case aF6  :StrTable.Jianxi[0]=0;break;
      case aF7  :F7(1);return;
/*返回*/  case aF8  :F7(0);KEYL=2;ShowKey(3,0);StrTable.Jianxi[0]=20;return;
      case LEFT :if(!XX)XX=13;XX--;ShowMess(XX+1);ClearF7();break;
      case RIGHT:XX++;if(XX>=13)XX=0;ShowMess(XX+1);ClearF7();break;
      case UP   :j=YY;YY--;if(YY==-1){YY=0;/*while(Table.Index[YY]==1)YY++;*/}ShowTable(j+1);ClearF7();break;
      case DN   :j=YY;if(Table.Index[j]==1)YY++;/*else YY=0;*/if(YY>14)YY=14;ShowTable(j+1);ClearF7();break;
    }
      }
      else{
    if(k==0xd){/*GetInput();*/
    SaveF7V();XX++;if(XX==13)XX=0;ShowMess(XX+1);ClearF7();}
    else{
      if(XX==1)Inputstr1[1]=0;
/*    ShowInput(k);*/
    }
      }
      ShowTable(YY+1);
    ...
    case 7: /*定段*/
      if(Fx){
    switch(k){
      case UP   :j=Dis_lines;j++;
        if(Dis_lines<0x10){if(Dis_lines)Dis_end=Dis_start=--Dis_lines;}
        else{if(Dis_lines>Dis_start+0x10)Dis_end=(--Dis_lines)&0xf;}
        ShowTable(j&0xf);break;
      case DN   :j=Dis_lines;j++;
        if(Dis_lines<0x10){if(Dis_lines<14)Dis_end=Dis_start=++Dis_lines;}
        else{if(Dis_lines<14+0x10)Dis_end=(++Dis_lines)&0xf;}
        ShowTable(j&0xf);break;
      case aF1:Fx=0;k=0xd;break;
      case aF2:
      case aF8:Dis_start=0;Dis_end=0;Dis_lines=0;KEYL=5;ShowKey(6,0);ShowTable(0);return;
        /*定段取消*/
    }
    ShowTable(1+Dis_lines&0xf);
      }if(!Fx){
    switch(k){
    case 0xd:if(Dis_lines<0x10){Dis_start=Dis_lines;Dis_lines+=0x10;}
      else{Dis_lines&=0xf;Dis_end=Dis_lines;Dis_lines=Dis_start;
      for(j=14;j;j--)if(Table.Index[j])break;
      if(Dis_end>j){Dis_end=j;ShowTable(0);}KEYL=5;ShowKey(6,0);}break;
    }
    ShowTable(1+Dis_lines&0xf);
      }
    break;
    case 8: /*口令*/
      if(Fx){
    switch(k){
      case aF1:SetPass(1,0xd);break;
      case aF2:
      case aF8:outb(OSC,0x01);
               OTPS0&=0x7f;
               outportb(OSC+1,OTPS0);DispF8(3);SetPass(2,0);KEYL=4;ShowKey(5,0);
               if(!F[1])F2();
               break;
      default:SetPass(1,0);
    }
    ...
    case 9:
      if(Fx){
    if(k!=aF1&&P[5]==1){P6();ShowKey(8,1);}
    if(k!=aF2&&P[6]==1){P7();ShowKey(8,2);}
    switch(k){
      case aF1:P6();ShowKey(8,1);break;
      case aF2:P7();ShowKey(8,2);break;
      case aF3:P8();ShowKey(8,0);break;
      case aF8:KEYL=3;ShowKey(4,0);break;
    }return;
     }
     if(!Fx&&k==0xd){
    if(P[5]==1)P6();
    ...
    switch(k){
      case DN:last_setline=setline;setline++;if(setline>7)setline=0;
            if(setline==0)
                ...
                {ListStr2(6,setline,1);ShowMess(41);}
            else if(setline==2)
                {ListStr3(6,setline,1);ShowMess(41);}
            else if(setline==3)
      ...
      case RIGHT:
          if(setline==0)
            {sl0++;if(sl0>1)sl0=0;ListStr1(6,setline,1);}
          else if(setline==1)
            {sl1++;if(sl1>1)sl1=0;ListStr2(6,setline,1);}
          else if(setline==2)
            {sl2++;if(sl2>1)sl2=0;ListStr3(6,setline,1);}
          else if(setline==3)
       ...
       case LEFT:
          if(setline==0)
            {if(sl0==0)sl0=1; else sl0--;ListStr1(6,setline,1);}
          else if(setline==1)
            {if(sl1==0)sl1=1; else sl1--;ListStr2(6,setline,1);}
          else if(setline==2)
            {if(sl2==0)sl2=1; else sl2--;ListStr3(6,setline,1);}
          else if(setline==3)
  ...
  case 11:  /*运动顺序设定*/
        switch(k){
      case RIGHT:
                sl7++;if(sl7>5)sl7=4;ListStr7(6,setline1,1,1);
                break;
      case LEFT:
                if(sl7==4)sl7=5; else sl7--; ListStr7(6,setline1,1,1);
                break;
      ...
      case aF1:
        ...
        break;/*确定*/
      case aF2:
      case aF8:DispF10(3);KEYL=1;ShowKey(2,0);break;/*取消*/
    }
  break;
  }
  if(!Fx)ShowInput(k);
...
void Down(void){
    long ll;
    if(Dis_flag||Ck8255_flag&0x20||(!K[3]&&(V<10&&!K[8])&&SWF_K<10)){ShowMess(17);return;}
    lock_z=0;
    if(!DN_K){UP_K=0;
    ClearKey(0);
    SoundTime=SoundTIME;
...
void Up(void){
    long ll;
    if(Dis_flag||Ck8255_flag&0x10||(!K[3]&&(V<10&&K[8])&&SWF_K<10)){ShowMess(17);return;}
    lock_z=0;
    if(!UP_K){DN_K=0;
    ClearKey(0);
    SoundTime=SoundTIME;
...
void Stop_z(char flag){
    ...
    long l;
    long l1=0;
    if(flag==-1){l=ZPosi(1);if(V<=10)flag=0;}
    if(flag!=-1){
    if(V<=10)flag=0;
     switch(flag){
        case 1:i=1;break;
...
void ClearF7(){
    ...
    switch(XX){
        ...
        case 1:MaxL=0;MaxN=0;MinN=0;break;
        case 2:
        if(sl5==0)
               {MaxL=5;MaxN=35000;MinN=0;break;}
        else
               {MaxL=5;MaxN=75000;MinN=0;break;}
        case 3: MaxL=3;MaxN=999;MinN=1;break;
        case 4: MaxL=3;MaxN=999;MinN=3;break;
        case 5: MaxL=3;MaxN=120;MinN=0;break;
        case 6: MaxL=1;MaxN=8;MinN=1;break;
        case 7: MaxL=3;MaxN=999;MinN=0;break;
        case 8: MaxL=2;MaxN=99;MinN=0;break;
        case 9: MaxL=1;MaxN=2;MinN=1;break;
        case 10:MaxL=2;MaxN=33;MinN=0;break;
        case 11:MaxL=2;MaxN=11;MinN=0;break;
        case 12:MaxL=2;MaxN=39;MinN=0;break;
