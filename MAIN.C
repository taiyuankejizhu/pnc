...
void InitKABT(){
    Pid_kp=0x14;Pid_ki=0x00;Pid_kd=0x04;Pid_st=0x0d;
    set_PID();
}
...
void P9(void){  /*钢蚀钢*/
    ...
    else{ClearKey(0);ShowMess(22);P[8]=1;MaxL=2;MaxN=99;MinN=1;}
    ShowFlag(7);
}
    void P10(void){ /*口令*/
    unsigned char a;
        ...
        if(l!=0x7FFFFFFF)
          {
            if(l==(long)321000)
              ...
              {ShowMess(30);if(!F[1])F2();}
          }
        else
          {ShowMess(0);if(!F[1])F2();}
      }
    else
      {
         ClearKey(0);
...
void P16(void){ /*系统设置*/
    ...
    if(P[14]){
        ...
        if(l!=0x7FFFFFFF){
            if(l==(long)1234000){
            if(!Dis_flag){KEYL=10;ShowKey(7,0);DispF9(0);}else ShowMess(17);return;
            }
            else{ShowMess(30);}
        }else{ShowMess(0);}
    }
    ...
    ShowMess(29);MaxL=7;MaxN=9999999;MinN=-9999999;P[14]=1;
    }
}
void P11(void){ /*修底*/
    ClearKey(0);
...
void F2(void){
    ...
    if(F[1]){
        ...
        IO3&=0x7f;outb(IO+3,IO3);/* 锁住 */
        ClosePower();
        F[1]=0;
    }else if(!ExitTime){
        IO3|=0x80;outb(IO+3,IO3);/* 开锁 */
        OpenPower();F[1]=1;
    }
    ShowFlag(1);
}
...
void Open_Elect(void){/*放电，开大变压器*/
    ...
    if(Table.Dianliu[Dis_lines]>7){outb(IO+4,IO4|=0x80);/*>4A开风扇*/
    FS=0x1000;  
    }
}
...
void OpenScr(void){
    ...
    if(Pum_flag&&!(Ck8256_flag&0x80))Pum_flag++;
    if(KeyPress())KeyAss();
    InitKey(1);
}
...
void F5(){
    ...
    StopF5();
        ...
        if(Ck8255_flag&0x3||Dis_flag||!F[1]){ShowMess(17);return;}
         {
         F[4]=2-K[8];}
    }
}
void Zero(){
    ...
    if(!F[4]){byte_2159=0;byte_215A=0;return;}
    lock_z=0;
...
void Position_control_z(long position)
    ...
    if(K[6])return;
        if(DZC==5){
        Pid_kp=0x20;Pid_ki=0x00;Pid_kd=0x04;Pid_st=0x40;}
    else{
        Pid_kp=0xa;Pid_ki=0x00;Pid_kd=0x04;Pid_st=0x0d;}
    Set_pid();
...
void Velocity_control_z(unsigned v)
    ...
    if(K[6])return;
        if(DZC==5){
        Pid_kp=0x1;Pid_ki=0x00;Pid_kd=0x04;Pid_st=0x40;}
    else{
        Pid_kp=0x1;Pid_ki=0x00;Pid_kd=0x04;Pid_st=0x0d;}
    Set_pid();
    outb(Address_Z,0x04);
...
{         
    if(!flag){IOZ0|=0x06;IOZ0&=0xfe;}
    else {IOZ0|=0x04;IOZ0&=0xfc;}
    outb(Address_Z,0x01);
    outb(Address_Z+1,IOZ0);
}
...
void F7(char flag){
    ...
    DelLine();
    i=F12_flag;j=Dis_lines;
    if(!Dis_flag)i=0;F12_flag=0;
    if(!Dis_flag){  
        SetTable(j);
        if(!K[8])F12();else F13();
    }else{
        TF8=1;
        if(K[6]){SetTable(j);Open_Elect();TF8=0;}/*锁定放电，调试模式*/
    }
    F12_flag=i;Dis_lines=j;
    /*ShowTable(YY+1);*/
        ShowTable(0);
    ShowTable(Dis_start+1);
    ShowDP(1);Write_Table(Group);
...
if(!flag){
    ...
    else {ClearKey(0);ShowMess(18);F[7]=1;MaxL=2;MaxN=99;MinN=1;ShowHT();}
}
else{
if(F[7]==1){
    if(KeyN!=0x7fffffff)F8H=KeyN;else{F8(0);return;}
    ShowMess(0);
    ShowMess(19);MaxL=2;MaxN=99;MinN=1;
    F[7]=2;
    ShowHT();
}else
if(F[7]==2){
...
else{ClearKey(0);ShowMess(21);K[2]=1;MaxL=2;MaxN=99;MinN=1;}
}
void K4(void){
static zu;
int i;
ClearKey(0);
if(Dis_flag){ShowMess(17);return;}
if(K[3]){K[3]=0;P[7]=2;KeyN=zu;F[8]=1;F9();}
else{zu=Group;KeyN=0;Read_Table(0);
    if(!K[8])
    Table.Shendu[0]=Table.Shendu[0];
    else
    Table.Shendu[0]=-Table.Shendu[0];
    F[8]=1;F9();K[3]=1;P[7]=0;
    }
}
char OilSwitch(){
    outb(OSC,0x01);
...
void F10(void){
    unsigned long gg;
    unsigned int i;
    if(!F[9]){ClearKey(0);if(Dis_flag){ShowMess(17);return;}
...
"\x00\x00\xFF\xFF\xFF\x00\xFF"};
    ...
    BMP[0x12]=800%256;BMP[0x13]=800/256;BMP[0x16]=600%256;BMP[0x17]=600/256;
...
void F12(void){
    ...
    if(F12_flag==0){
        XD=0;
        if(Dis_flag){
    if(Ck8255_flag&0x3||!F[1]){ShowMess(17);F11();return;}
        for(i=Dis_lines;i<=Dis_end;i++){
            if(Table.Shendu[i]>position_z){Dis_lines++;continue;}
            if(Table.Index[i]!=1){Dis_lines++;continue;}
            break;
        }
        if(Dis_lines>Dis_end&&Dis_flag){
            ...
            offset_ds=0;
            if(!Table.Index[Dis_lines]){ShowMess(32);F11();return;}
            else ShowMess(28);
            if(!Dis_first){F11();return;}
            F11();offset_rz=ZPosi(0);
            if(K[1]){K[9]=1;ExitTime=360;if(KEYL==4)ShowKey(5,4);}
            if(K[2])
                offset_rz+=((long)K_3*1000/DZC);
                Position_control_z(offset_rz);
            return;
        }
        if(Dis_lines<=Dis_end){ShowTable(Dis_lines+1);
                SetTable(Dis_lines);
                DY=Table.Jianxi[Dis_lines]/2+41+Table.Gaoya[Dis_lines]%10*15;
        }
        ...
        lh*=5/DZC;
        F12_flag++;
        AI=0;AITime=0;AIHigh=0;positiond1=9999999/DZC;
        }Dis_first=1;
        return;
    }
    if(DIS_OK!=1&&DIS_OK1!=1){ShowMess(33);F11();}
    if(Ck8255_flag&0x3||!F[1]){F11();return;}
    if(!Dis_flag)return;
    switch(F12_flag){
    case 0:break;
    case 1:
        ShowTable(0);ShowFL(0);
        if(Voltage()<10){Velocity_control_z(0x40);Delay1(0x100);}Position_control_z(ZPosi(0));
        ShowTable(Dis_lines+1);
        end=0;
        ZeroFlag=0;ShowTB(0);
        SetTable(Dis_lines);
        ...
        Open_OCSF();
        outb(OSC,0x01);
        outb(OSC+2,OTPS1&=0xfe);   /*DIS LED*/
        GetTime((long)0);offset_t=Oldtime/18.2;
        if(F[2]){Open_Pump();ShowFlag(2);}
        if(K[6]){ShowFL(2);ShowFL(5);F12_flag=9;SetDianliu(Table.Dianliu[Dis_lines]);ShowDP(0);return;}
        Delay1(0x100);
        SetDianliu(1);
        Signal_Jianxi(K[8]);
        F12_flag++;
    ...
    case 2:
        ...
        if(P[3])Close_OCSF();
            ...
            ldv=positiond+ld;
            if(ldv<ZPosi(0))Position_control_z(ldv);
            F12_flag=4;
            ia=3;
            ShowDP(1);
    ...
    case 3:
        ...
        Dis_lines++;
        if(Dis_lines>Dis_end){F12_flag=0;return;}
        SetTable(Dis_lines);
        DY=Table.Jianxi[Dis_lines]/2+41+
        Table.Gaoya[Dis_lines]%10*15;
        ShowTable(0);
        ShowTable(Dis_lines+1);return;
        }
        ShowFL(5);
        position_d=position_z;
        if(Voltage()<=Table.Jianxi[Dis_lines]){/*Delay1(50);*/
        if(Voltage()<=Table.Jianxi[Dis_lines])F12_flag+=2;
        ShowFL(0);
        }
        ShowDP(1);
    ...
    case 4:
        ...
        if(P[10]<=1){/*NO 修底*/
        Signal_Jianxi(K[8]);
        if(ia||ERR_XY){position_d=position_z;ShowFL(0);ShowFL(4);ShowFL(8);ia=0;positiond1=positiond;}
        }else F12_flag++;
        ERR_XY=0;
        GetTime((long)0);}
                break;
    case 5:
        ...
        if(Dis_lines>Dis_end){
            if(!P[10]){F12_flag=0;return;}
            else{       /*修底*/
            Dis_lines--;P[10]++;ShowMess(22);vol=0xff;
            positiond=position_p-offset_z;
            ld=0;F12_flag++;return;
            }
        }
        SetTable(Dis_lines);
        DY=Table.Jianxi[Dis_lines]/2+41+Table.Gaoya[Dis_lines]%10*15;
        ...
        ld=Table.Dianliu[Dis_lines]/8+MianJi;/* +5 */
        ld*=5/DZC;
        lh=Table.Shenggao[Dis_lines]*20+ld+10;
        lh*=5/DZC;
        }
        }else end=0;
        if(Voltage()>DY)return;
        if(Table.Shenggao[Dis_lines]>0)F12_flag++;
        break;
    case 6:
        ShowFL(5);ERR_XY=0;
        if(P[10]>1&&end>1){/*修底*/
            ...
            delay(100);
            if(Voltage()+10>vol&&ZPosi(0)+offset_z<=position_p)XD++;
            if(XD>5){F12_flag=0;Dis_lines++;return;}
            }
        ...
        if(P[10]<=1)positiond=ZPosi(0);
        if(offset_d>positiond)offset_d=positiond;
        if(positiond<positiond1)positiond1=positiond;
        if(positiond>positiond1+1){if(!AI)AI=1;AIHigh=positiond-positiond1-1;
        }else if(AI){if(AI==2)ClearAI(1);AI=0;AITime=0;AIHigh=0;}
        offset_ds=1;
    if(GetTime(Work_time)||P[10]>1){
        Close_OCSF();
        ShowFL(6);
        lhv=positiond+MianJi/DZC;
        if(P[10]<=1)positiond+=45/DZC;
        F12_flag++;
        }
        ...
        Velocity=(120-Velocity)*5;
        Velocity=abs(Velocity);
        break;
    case 7:
          Velocity_control_z(Velocity);
          if(Velocity_GetTime(V_time)||Zposi(0)>=(positiond+lh))
          {
             Position_control_z(lhv+100/DZC);
             if(ZPosi(0)>lhv){
             if(F[7]==3){if(++ht>=F8T){ht=0;hth=(long)F8H*1000/DZC;}
             else{hth=0;}}else{ht=0;hth=0;}
    ...
    case 8:
        ...
        if(ZPosi(0)>lhv-50/DZC){
            ...
            if(K[4]){
            if(OilSwitch()){ShowMess(27);Oil=1;return;}
            if(Oil){Oil=0;ShowMess(0);}
            }
        if(Steel_flag){if(++steel==Steel_flag){JiXin(JX);steel=0;JX^=1;}}
        ldv=positiond+ld;
        F12_flag=4;
        ia=2;
        if(!P[3]||P[10]>1)Open_OCSF();
        ii=Voltage();ii+=Voltage();
        ii+=Voltage();ii+=Voltage();
        vol=ii/4;
        if(vol>0xfe-10)vol=0xfe-10;
        if(!ERR_XY)Position_control_z(ldv);
        }
                break;
    case 9: break;
    }
...
void F13(void){
    ...
    switch(F12_flag){
    case 0:
    case 1: ShowTable(0);ShowFL(0);
        if(Voltage()<10){Velocity_control_z(0xff80);Delay1(0x100);}
        Position_control_z(ZPosi(0));
        ShowTable(Dis_lines+1);
        end=0;
        ZeroFlag=0;ShowTB(0);
        ...
        F12_flag++;
        break;
    case 2: if(offset_d>ZPosi(0)/*&&!K[3]*/&&offset_ds&&0){
        if(P[3])
            Close_OCSF();
            positiond=offset_d-545/DZC;
            ldv=positiond-ld;
...
/*      Position_control_z(ldv);*//*7-15*/
    ...
    case 5: 
        ...
        if(Voltage()>DY)return;
        if(Table.Shenggao[Dis_lines]>0)F12_flag++;break;
    case 6:ShowFL(5);ERR_XY=0;
        if(P[10]>1&&end>1){/*修底*/
            position_d=position_p;
            if(Voltage()+10>vol&&ZPosi(0)+offset_z>=position_p){
            delay(100);
            if(Voltage()+10>vol&&ZPosi(0)+offset_z>=position_p){F12_flag=0;Dis_lines++;return;}
            }
            if(GetTime(Work_time)){
            Close_OCSF();
            ShowFL(6);
        ...
        if(offset_d<positiond)offset_d=positiond;
        if(positiond>positiond1)positiond1=positiond;
        if(positiond<positiond1-1){if(!AI)AI=1;AIHigh=0-(positiond-positiond1-1);
        }else if(AI){if(AI==2)ClearAI(1);AI=0;AITime=0;AIHigh=0;}
        offset_ds=1;
    if(GetTime(Work_time)||P[10]>1){
        Close_OCSF();
    ...
    case 8: ShowFL(7);
        ...
        ii=Voltage();ii+=Voltage();
        ii+=Voltage();ii+=Voltage();
        vol=ii/4;if(vol>0xfe-10)vol=0xfe-10;
        if(!ERR_XY)Position_control_z(ldv);
        }
        break;
    case 9: break;
...
int ProTime(void){
    ...
    if(j>=1){
        ...
        {  ShowPZ(3);
            if(ExitTime>36){ExitTime=36;if(F[1])F2();}
        }
    if(FS&&FS<0x800){
        if(FS<=l1){FS=0;outb(IO+1,IO1&=0xbf);/*关风扇*/}
        else FS-=l1;
    }
    if(ExitTime){
        ShowMess(31);
...
void ClearAI(int flag){
    ...
    if(flag==1){
        SetTable(Dis_lines);AI=0;AIHigh=0;AITime=0;
    }
    if(flag==2)
    {
        ia=AIHigh>100 ? 100 : AIHigh;
        ia=AIHigh<0 ? 0 : AIHigh;
            
        Work_time=Table.Gongshi[Dis_lines]/5 +
            (Table.Gongshi[Dis_lines]*4*(100-ia))/500;

        i=Table.Maikuan[Dis_lines];
        j=Table.Xiuzhi[Dis_lines]/5 +
            (Table.Xiuzhi[Dis_lines]*4*ia)/500;
        Duty_ratio(i,j,0);          /* 脉冲 */
        MianJi=(1); /* 面积 */
    }
}
...
void SetDianliu(unsigned dianliu){
    ...
    outb(IO+1,IO1);     /* 电流 */
        ...
        if(Table.Dianliu[Dis_lines]>7&&Dis_flag){
...
void SetTable(char lines){
    ...
    IO3&=0xfc;outb(IO+3,IO3);           /* 停高压V */
    if(GaoyaV==0){IO3|=0x00;Gaoya=160;}
    if(GaoyaV==1){IO3|=0x00;Gaoya=160;}
    if(GaoyaV==2){IO3|=0x01;Gaoya=200;}
    if(GaoyaV==3){IO3|=0x03;Gaoya=260;}
    outb(IO+3,IO3);                     /* 高压V */

    GaoyaA=Table.Gaoya[lines]%10;
    IO3&=0xf3;outb(IO+3,IO3);           /* 停高压A */
    if(GaoyaA==0)IO3&=0xf3;
    if(GaoyaA==1)IO3|=0x04;
    if(GaoyaA==2)IO3|=0x08;
    if(GaoyaA==3)IO3|=0x0c;
    outb(IO+3,IO3);                     /* 高压A */
    ...
    */
    ...
    IO2&=0x1f;outb(IO+2,IO2);
    if(WcL==0)outb(IO+2,IO2&=0x1f);
    if(WcL==1)outb(IO+2,IO2|=0x80);
    if(WcL==2)outb(IO+2,IO2|=0x20);
    if(WcL==3)outb(IO+2,IO2|=0x40);
    WcC=Table.WcLc[lines]%10;                               /* 硬质合金 */
    IO2&=0xe0;outb(IO+2,IO2);
    switch(WcC)
    {
    case 0:
        IO2&=0xe0;
...
void Show_hint(void){
 ...
 if(((b&0x80)||(b&0x40)||(DIS_OK!=1)||!(k&0x2))&&i)
    {
       setfillstyle(1,0x8);
       bar(LX0+4,LY0+22,LX1-1,LY0+56);
       List16x16M(LX0+1,LY0+27,0xf,"");
      i=0;
     }
}
